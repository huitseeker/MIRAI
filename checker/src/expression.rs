...
pub enum Expression {
    ...
    /// the path of this value. This models a variable that is assigned from inside a loop
    /// body.
    Widened {
...
impl Expression {
    ...
    pub fn infer_type(&self) -> ExpressionType {
        ...
        match self {
            ...
            Expression::Joined { left, .. } => left.expression.infer_type(),
            ...
            Expression::Widened { operand, .. } => operand.expression.infer_type(),
        }
    }
