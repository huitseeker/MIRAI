        ...
        match expression {
            ...
            Expression::Widened { path, operand } => {
                ...
                unsafe {
                    ...
                    let domain: AbstractDomain = AbstractDomain::from(expression.clone());
                    let interval = domain.get_as_interval();
                    if !interval.is_bottom() {
                        checked_assume!(expr_type.is_integer());
            ...
            Expression::Joined { .. } | _ => unsafe {
                use self::ExpressionType::*;
                //info!("uninterpreted expression: {:?}", expression);
                let sort = match expression.infer_type() {
                    Bool => self.bool_sort,
                    Char | I8 | I16 | I32 | I64 | I128 | Isize | U8 | U16 | U32 | U64 | U128
                    | Usize => self.int_sort,
                    F32 => self.f32_sort,
                    F64 => self.f64_sort,
